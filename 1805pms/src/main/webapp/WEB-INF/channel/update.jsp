			...
			<tbody id="tab_1">
				...
				<tr>
					...
					<td width="80%" class="pn-fcontent">
						<input type="text"  class="required" name="cname"id="cname" onblur="chkcname()" value="${channel.cname }"/>	
					...
					<td width="80%" class="pn-fcontent">
					   <select name="pid">
							<c:forEach items="${pnames }" var="pnames" >
								<c:if test="${channel.pid==pnames.id }">
								   <option value="${pnames.id }" selected="selected">${pnames.pname}</option> 
								</c:if>
								<c:if test="${channel.pid!=pnames.id }">
								   <option value="${pnames.id }" >${pnames.pname}</option>	
								</c:if>
							</c:forEach>			
						</select>
					</td>
				...
				<tr>
					...
					<td width="80%" class="pn-fcontent">
					  ...
					  <c:if test="${channel.isleaf==2 }">
						<input type="radio" name="isleaf" value="1" />是叶子
						<input type="radio" name="isleaf" value="2" checked="checked"/>不是叶子
					  </c:if>
					</td>
				</tr>
